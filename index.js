...
function isItAnApple(strings) {
  const newStrings = strings.map(function(item){
    if(item=='apple'){
      return true;
    }
    else{
...
function removeApple(strings) {
  const newStrings = strings.filter(function(item){
    if(item!='apple'){
      return item;
    }
  })
...
function stringSmash(strings) {
  const newStrings = strings.reduce(function(accumulator, item){
...
function getFullNames(runners) {
  ...
  runners.forEach(function(item){
    return runnersName.push(item.last_name + ", "+item.first_name);
  })
  return runnersName;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const tShirtOrder=runners.filter(function(item){
